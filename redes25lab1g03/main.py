...
def obtener_peliculas() -> Response:
    ...
    Parámetros_de_consulta:
        search(str, opcional): Cadena de texto para filtrar películas por título. Si no se proporciona, se devuelven todas.

    :returns Response peliculas_enviar: Un objeto JSON con la lista de         películas filtradas por nombre completa.
    """
...
def obtener_pelicula(id: int) -> Response:
    ...
    :returns tuple ((jasonify(pelicula_encontrada)), 200): Tupla con un objeto json con la pelicula encontrada y un codigo de solicitud de que está todo bien.
...
def agregar_pelicula() -> Response:
    ...
    :returns tuple ((jsonify(nueva_pelicula)), 201): Tupla con un objeto json con la pelicula nueva y un codigo de solictud de que se creó con éxito.
    """
    try:
...
def actualizar_pelicula(id: int) -> Response:
    ...
    :returns Response act_peli: Un objeto json que contiene la película actualizada.
...
def obtener_pelicula_random_genero(genero: str) -> Response:
    ...
    if list_pelicula == []:
        print(
            "No existen películas del genero {genero_norm} entre las disponibles.")
        return jsonify([]), 200
...
def dar_sugerencia() -> Response:
    ...
    :returns tuple ((jsonify(suggested_movie)), 200): Un tupla con un objeto JSON que contiene la película recomendada y un codigo de solicitud. 

    :raises OK: Salió todo bien.
    """
    # si peliculas esta vacia entonces devuelvo un error
    if not peliculas:
        return jsonify({"error": "No hay peliculas disponibles para sugerir"}), 404
