...
def test_obtener_pelicula_recomendada_para_prox_feriado_genero_ciencia_ficcion(mock_response):
...
def test_obtener_pelicula_recomendada_para_prox_feriado_tipo_trasladable_genero_accion(mock_response):
    response = requests.get(
        f'{URL_API}/sugerencia_feriado?genero=accion&tipo=trasladable')
    assert response.status_code == 200
...
def test_error_obtener_pelicula_recomendada_para_prox_feriado_no_genero(mock_response):
